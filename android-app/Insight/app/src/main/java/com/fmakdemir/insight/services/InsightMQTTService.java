 ...
 *    28 Jan 2011
 */
public class InsightMQTTService extends Service implements MqttSimpleCallback
{
	/************************************************************************/
    /*    CONSTANTS                                                         */
	/************************************************************************/
...
//	private BackgroundDataChangeIntentReceiver dataEnabledReceiver;
	...
	public void onCreate()
	{
		...
		mBinder = new LocalBinder<InsightMQTTService>(this);
		...
		brokerHostName = "iot.eclipse.org";
		topicName      = settings.getString("topic",  "/insight/android");
...
/*		dataEnabledReceiver = new BackgroundDataChangeIntentReceiver();
	...
	synchronized void handleStart(Intent intent, int startId)
	{
		...
		if (!isAlreadyConnected())
		{
			...
			notifyUser("InSight", "MQTT Started", true);
...
*/
	...
	private LocalBinder<InsightMQTTService> mBinder;
...
/*			mqttClient.publish(topicName,
		...
		catch (MqttException e)
		{
			...
			Log.e(Helper.getTag(this), "Unable to connect to broker: "+brokerHostName);
		}
	}
	...
	private boolean connectToBroker()
	{
		try
		{
			...
			Log.w(Helper.getTag(this), brokerHostName+":"+BROKER_PORT_NUMBER+"\n"+topicName);
